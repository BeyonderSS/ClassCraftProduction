...
console.log(selectedLecture)
  ...
  const handleGenerateLectures = async () => {
    try {
      ...
      setError("Failed to Create a Lecture. Please try again later.");
    } finally {
      setMeetingData({
        topic: "",
        location: "Online",
  ...
  const filteredLectures = fetchedLectures.filter(
    (lecture) => lecture.subjectId === props.params.subjects
  );
  useEffect(() => {
    if (session) {
  ...
  // Filter lectures based on the subject ID from props
